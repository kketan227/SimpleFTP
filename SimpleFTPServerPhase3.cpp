...
int main (int argc, char **argv)
{
 ...
 if(bind (listenfd, (struct sockaddr *) &servaddr, sizeof(servaddr))<0){
	...
	else cout<<"bind done :"<<argv[1]<<endl;
 ...
 for ( ; ; ) {
  ...
  //cout<<"Child created for dealing with client requests "<<data_port<<" is the data port"<<endl;
	cout<< "Client: " << inet_ntoa(cliaddr.sin_addr) << ":" << ntohs(cliaddr.sin_port) <<"\n";
  ...
  while ( (n = recv(connfd, buf, MAXLINE,0)) > 0)  {
   ...
   if (strcmp("put",token)==0)  {
	...
	if(strcmp("1",check)==0){
		...
		else
		{
			...
			cout<<num_blks*MAXLINE+num_last_blk<<"bytes of data written.\n";
		}
	}
   }
   ...
   if (strcmp("get",token)==0)  {
	...
	if ((fp=fopen(token,"r"))!=NULL)
	{
		...
		fclose(fp);
		cout<<"Transfer done: "<<lSize<<endl;
		
	}
	else{
		send(connfd,"0",MAXLINE,0);
		cout<<"File transfer failed. File not present/readable."<<endl;
	}
   }
